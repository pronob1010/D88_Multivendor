...
import SingleMenuItem from './mega-menu/single-menu-item';
...
export default function HeaderAll() {
    ...
    const { username, is_vendor } = useSelector(state => state.userReducer.user_details)

    // console.log(username, is_vendor);
    ...
    let menu_items = categories_list.map(item => {
        return (<SingleMenuItem item={item} key={item.id} />)
    })
    ...
    if (is_vendor == true) {
         ...
         store = (
            <li className="list-inline-item mr-0 u-header-topbar__nav-item u-header-topbar__nav-item-border" >
                <a href="#" className="u-header-topbar__nav-link"><i className="ec ec-map-pointer mr-1"></i> Store Locator</a></li>)
